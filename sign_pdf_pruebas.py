...
# Variables globales
ruta_pdf = None  # Inicializamos ruta_pdf como None
img = None
pos_x, pos_y = None, None

# Función para abrir el PDF y convertir la primera página en imagen
def abrir_pdf():
    global img, ruta_pdf  # Declaramos la variable global
    pdf_path = filedialog.askopenfilename(filetypes=[("PDF files", "*.pdf")])
    if pdf_path:
        ruta_pdf = input("pdf_path: ")  # Asignamos la ruta del PDF
...
def colocar_firma():
    global pos_x, pos_y, ruta_pdf
    if ruta_pdf is None:
        print("No se ha seleccionado ningún PDF.")
        return
...
canvas_pdf = tk.Canvas(ventana, width=600, height=800, bg="white")
canvas_pdf.pack()
...
boton_firma.pack(pady=10)

# Evento de clic para obtener la posición
